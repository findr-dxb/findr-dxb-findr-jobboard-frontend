...
import {
  ...
  type ActiveUserTodayItem,
  type ActiveUsersTodayList,
} from "@/lib/admin-api"
...
export default function ActiveUsersTodayPage() {
  const [data, setData] = useState<ActiveUsersTodayList | null>(null)
  const [isLoading, setIsLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [page, setPage] = useState(1)

  const loadActiveUsers = useCallback(async (pageNum: number) => {
    try {
      setIsLoading(true)
      setError(null)
      const result = await getActiveUsersTodayList({ page: pageNum, limit: 20 })
      setData(result)
    } catch (err) {
      console.error("Error loading active users today:", err)
      setError("Failed to load recent logins")
    } finally {
      setIsLoading(false)
    }
  }, [])
  ...
  const users: ActiveUserTodayItem[] = data?.users || []
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Active users today · {todayLabel}</p>
            </div>
          </div>
        </div>
            ...
            <table className="w-full min-w-[720px] text-left">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/80 text-xs font-medium uppercase tracking-wide text-gray-400">
                  ...
                  <th className="px-5 py-3.5 font-medium text-right">Last Login</th>
                </tr>
              </thead>
              <tbody>
                {users.map((user) => (
