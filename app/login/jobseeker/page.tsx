...
export default function JobSeekerLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await login(formData.email, formData.password, "jobseeker")
      if (success) {
        toast({
          title: "Login Successful!",
          description: "Welcome back! Redirecting...",
        })
        router.push(redirectUrl || "/jobseeker/dashboard")
      }
    } catch (error: any) {
      ...
      <div className="flex items-center justify-center py-16 px-4">
        <Card className="w-full max-w-md border-2 border-emerald-200">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl">Job Seeker Login</CardTitle>
            <CardDescription>Sign in to access your dashboard and find your dream job</CardDescription>
          </CardHeader>
          <CardContent>
