...
type ProfileAccessClientProps = {
  token: string | null
}

export default function ProfileAccessClient({ token }: ProfileAccessClientProps) {
    ...
    const confirm = async () => {
      try {
        const res = await fetch(
          `${API_BASE_URL}/profile-access/confirm?token=${encodeURIComponent(token)}`
        )
